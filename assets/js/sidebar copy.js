document.addEventListener("DOMContentLoaded", function() {
    ...
    function populateDays(event) {
        ...
        // Listen for day selection to jump to article
        daySelect.addEventListener("change", () => {
            jumpToArticle(selectedYear, selectedMonth, daySelect.value);
        });
    }
    ...
    // Function to jump to the article based on selected date
    function jumpToArticle(year, month, day) {
        const formattedDate = `${month} ${day}th ${year}`;
        const articleDateElements = document.querySelectorAll(".article-data span");
        articleDateElements.forEach(element => {
            if (element.textContent === formattedDate) {
                // Scroll to the article
                element.closest(".blog-post-data").scrollIntoView({ behavior: "smooth" });
            }
        });
    }
});
